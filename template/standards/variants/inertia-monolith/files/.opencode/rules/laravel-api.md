...
globs: app/**,routes/**,config/**,database/**,tests/**,Modules/**
alwaysApply: false
---

# Laravel

Laravel lives in the app tree that owns `artisan`. OpenCode is the laptop process harness. Laravel Boost is the Laravel agent layer.
...
- Repo-root MCP starts Boost with `php` and `artisan boost:mcp` in `opencode.json`. Do not set a working directory on that command.
