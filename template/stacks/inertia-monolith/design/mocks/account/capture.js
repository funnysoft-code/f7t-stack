...
for (const direction of ["a", "b"]) {
  for (const [viewport, size] of [
    ...
    for (const screen of screens) {
      for (const state of states) {
        ...
        const metrics = await page.evaluate(() => ({
          ...
          inputsLabeled: [...document.querySelectorAll("input")].every(
            (input) => input.labels?.length > 0,
          ),
    ...
    await page.goto(`http://127.0.0.1:4187/index-${direction}.html?screen=deletion`, {
      waitUntil: "load",
    });
    ...
    evidence.push({
      ...
      screenshot: await saveScreenshot(
        ...
        `f7t-${direction}-deletion-dialog-${viewport}.png`,
      ),
    });
...
console.log(
  JSON.stringify({
    verifiedViews: evidence.length,
    screenshots: evidence.filter((row) => row.screenshot).length,
    fullScreenshots: evidence.filter((row) => row.fullScreenshot).length,
  }),
);
