...
For a secured Herd proxy in front of Next, set both frontend origins to the exact public HTTPS hostname and keep the API upstream private. Start development Next with that explicit hostname, for example `bun run dev --hostname __F7T_APP_NAME__.test --port 3000`, so its development asset origin checks recognize the host. Do not disable origin checks or allow every development origin. Use `bun run build` followed by `bun run start` when checking production response caching.

Laravel's `TRUSTED_PROXIES` defaults to empty for a direct private API listener. If a TLS-terminating reverse proxy fronts Laravel itself, list only its actual peer IPv4/IPv6 addresses, separated by commas. A local loopback Herd proxy can use `127.0.0.1`, with `::1` only when used. Wildcards, hostnames, CIDR ranges and `REMOTE_ADDR` are rejected. Restrict access to the backend, preserve the public Host at the proxy and overwrite `X-Forwarded-Proto` from the incoming transport. Laravel trusts only that scheme header from those peers, not forwarded Host, port or client IP headers. Rebuild cached configuration and restart workers after changes. The Next API adapter intentionally does not forward browser-supplied proxy headers.
...
Production Next sends `private, no-cache, no-store, max-age=0, must-revalidate` for these dynamic account/auth pages. Development Next can replace that with `no-cache, must-revalidate`; its HTML headers do not establish the production cache policy. Check authenticated HTML through `next start`, including redirects and error states, while leaving immutable public assets cacheable. API and Horizon responses retain their explicit `private, no-store` policy.
